...
package ssmit
{
	...
	public final class FlashMovieClipImporter
	{
		...
		public static function importFromXMLAssetManager(animacaoXML:XML, assets:AssetManager ) : ConvertedMovieClip
		{
			
			var version:String = animacaoXML.@version;	// TODO: Check version
			var frameRate:Number = animacaoXML.@frameRate;
			
			// Create the objects.
			var objects:Vector.<DisplayObject> = new Vector.<DisplayObject>( animacaoXML.objects[0].object.length(), true );
			for(var i:int =0; i<objects.length; ++i )
			{
				var objectXML:XML = animacaoXML.objects[0].object[i];
				var objectType:String = objectXML.@type;
				var object:DisplayObject;
				switch( objectType )
				{
					...
					case "sprite":
						...
						break;
					case "image":
						object = createImageAssetManager( objectXML, assets );
						break;
					default:
						throw new Error( "Unhandled object type: " + objectType );
				}
			...
			for( i=0; i<objects.length; ++i )
			{
				objectXML = animacaoXML.objects[0].object[i];
				objectType = objectXML.@type;
				if( objectType == "movie clip" )
				{
					// Fill in the frame data and scene data.
			...
			// Return the first object, which is the root movie clip.
			movieClip = ConvertedMovieClip(objects[ 0 ]);
			// movieClip.textureAtlases = assets;
			
			return movieClip;
		}		
		
		private static function createImageAssetManager( imageXML:XML, assets:AssetManager ) : Image
		{
			var textureXML:XML = imageXML.texture[0];
			var atlasIndex:int = textureXML.@atlas;
			var regionName:String = textureXML.@region;
			var texture:Texture = assets.getTexture(regionName);
